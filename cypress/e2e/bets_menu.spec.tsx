...
describe("Bets Menu", () => {
  ...
  it("should open to a specific bet if a bet item is clicked", () => {
    cy.get('button[title="Bets"]').click().wait(2000)
  ...
  it("should open to persona if user tile is clicked", () => {
    ...
    cy.get('button[id="user-bet-data-button"]').first().click().wait(2000)
  ...
  it("should mark a bet as read if it is clicked", () => {
    cy.callFirestore("set", "lobby/testid", {
      ...
      user1Id: Cypress.env("CYPRESS_TEST_UID"),
      ...
      user2Id: Cypress.env("CYPRESS_TEST_UID_2"),
